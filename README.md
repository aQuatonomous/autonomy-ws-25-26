...
- **Scripts kept:**  
  - `comp.sh` — full competition run: sets camera FPS, then starts LiDAR + CV pipelines.  
...
- **“Camera X not found” / wrong camera order:** Run `./monitor_camera_move.sh` and align the listed by-path ports with Cam0/1/2 in `computer_vision/set_camera_fps.sh`. Override at launch if needed: `CAMERA_DEVICES="/path1,/path2,/path3" ./comp.sh`.
