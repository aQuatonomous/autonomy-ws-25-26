...
echo "=== Task 2: Setting camera format (single camera: YUYV @ 960x600 @ 15fps) ==="
...
sleep 2
echo "=== Launching CV pipeline (Task 2: camera1, task:=2, indicator_buoy:=true) ==="
...
sleep 2
echo "=== Starting Task 2 global planner (task_id:=2) ==="
ros2 launch global_planner global_planner.launch.py task_id:=2 cmd_vel_topic:=/uas1/mavros/setpoint_velocity/cmd_vel_unstamped &
PLANNER_PID=$!

start_sound_pipeline

echo "=== Task 2 pipelines started. MAVROS: $MAVROS_PID  GLOBAL_FRAME: $GLOBAL_FRAME_PID  LiDAR: $LIDAR_PID  CV: $CV_PID  FUSION: $FUSION_PID  PLANNER: $PLANNER_PID ==="
