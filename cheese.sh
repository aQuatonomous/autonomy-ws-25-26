...
# Same stack as task3 (camera, global_frame, LiDAR, CV, fusion) but no planning nodes.
# Mission: set home, waypoint 20 m ahead via MAVROS setpoint_position/global, then return home.
...
echo "=== Cheese: Setting camera format (single camera: YUYV @ 960x600 @ 15fps) ==="
...
cleanup() {
    echo ""
    echo "=== Stopping MAVROS, global_frame, LiDAR, CV, fusion, and mission ==="
    [ -n "$MISSION_PID" ] && kill -TERM "$MISSION_PID" 2>/dev/null || true
...
sleep 2
echo "=== Starting evacuation mission (home -> 20 m forward -> return home, no planning node) ==="
echo "Ensure FC is in GUIDED (or equivalent) mode and armed. Press Ctrl+C to stop."
python3 "${SCRIPT_DIR}/cheese_mission.py" &
MISSION_PID=$!

echo "=== Cheese pipelines started. MAVROS: $MAVROS_PID  GLOBAL_FRAME: $GLOBAL_FRAME_PID  LiDAR: $LIDAR_PID  CV: $CV_PID  FUSION: $FUSION_PID  MISSION: $MISSION_PID ==="
