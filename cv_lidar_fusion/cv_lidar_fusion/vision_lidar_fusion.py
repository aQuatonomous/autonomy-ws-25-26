...
Matches by angle: CV provides angle_rad per detection; LiDAR provides bearing (radians).
...
class VisionLidarFusionNode(Node):
    """Fusion node: match LiDAR tracks to CV detections by angle, assign color, publish /fused_buoys."""
    ...
    def _cv_callback(self, msg: String) -> None:
        """Parse CV JSON and store detections (combiner provides angle_rad per detection)."""
        try:
    ...
    def _run_fusion(self, msg: TrackedBuoyArray) -> None:
        ...
        CV angle_rad: 0=center, negative=left, positive=right. LiDAR bearing: 0=forward, positive=left.
        So cv_bearing = -angle_rad for comparison.
        """
        ...
        # Build list of CV detections with valid angle_rad and buoy class_id; compute cv_bearing
