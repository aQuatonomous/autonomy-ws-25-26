...
- **Gazebo Sim (gz sim)** runs the **Aquatonomous** boat world (`aquatonomous_world.sdf` in `src/asv_wave_sim/gz-waves-models/worlds/`) with water, buoys, gates, and the **ourboat** model.
...
- **Gazebo** loads the world `aquatonomous_world.sdf` (in `src/asv_wave_sim/gz-waves-models/worlds/`) with water (waves model), buoys, gates, and the ourboat model.
...
| What | Path |
|------|------|
| **This doc** | `autonomy-ws-25-26/src/asv_wave_sim/AQUATONOMOUS_SIM.md` |
| **Launch script** | `src/asv_wave_sim/aquatonomous_simulation_full.bash` |
| **World (map)** | `autonomy-ws-25-26/src/asv_wave_sim/gz-waves-models/worlds/aquatonomous_world.sdf` |
...
1. **From the repo (recommended):**
   ...
   bash src/asv_wave_sim/aquatonomous_simulation_full.bash
   ```
...
2. **From anywhere** (single canonical script in the repo):
   ...
   ```
   Optionally symlink for a shorter command: `ln -s ~/autonomy-ws-25-26/src/asv_wave_sim/aquatonomous_simulation_full.bash ~/simulation_full.bash`
...
**1. Start the simulation (from repo root):**
```bash
cd ~/autonomy-ws-25-26
bash src/asv_wave_sim/aquatonomous_simulation_full.bash
```
...
- **Faster inference (higher FPS):** Rebuild the TensorRT engine with **FP16** for roughly 2× speed with minimal accuracy loss. From the computer_vision repo (see `models/TENSORRT.md`):
  ```bash
  ...
  ```
  Ensure `engine_path` in your launch points at the new `model.engine`. INT8 is faster still but requires calibration.
...
- **Fix:** On the machine where `~/ardupilot` lives (e.g. the Jetson), reset ownership and do a clean SITL rebuild (do **not** use `sudo` for builds):
  ...
  Or run the helper script from this repo (on that machine): `src/asv_wave_sim/fix_ardupilot_build_permissions.sh`.
...
- **Fix:**
  ...
  2. **Paths:** Ensure on the Jetson you have: `~/autonomy-ws-25-26/src/asv_wave_sim/gz-waves-models/worlds/aquatonomous_world.sdf`, `gz-waves-models/models/`, `gz-waves-models/world_models/`, `~/SITL_Models/Gazebo/models`, `~/ardupilot_gazebo/models`. Same layout under `$HOME` as on the machine where the script was written.
...
- **One script:** `src/asv_wave_sim/aquatonomous_simulation_full.bash` starts Gazebo, bridges, MAVROS, and SITL in a tmux session.
- **One world:** `aquatonomous_world.sdf` in `src/asv_wave_sim/gz-waves-models/worlds/`.
