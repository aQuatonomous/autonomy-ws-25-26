...
send "${SESSION}:0.0" "source /opt/ros/jazzy/setup.bash && source ${ASV_INSTALL}/setup.bash && export GZ_VERSION=harmonic && export GZ_CONFIG_PATH=/usr/share/gz:\${GZ_CONFIG_PATH:-} && export LD_LIBRARY_PATH=${GZ_WAVES_LIB}:${ARDUPILOT_GAZEBO_ROOT}/build:\$LD_LIBRARY_PATH && export GZ_SIM_SYSTEM_PLUGIN_PATH=${GZ_WAVES_LIB}:${ARDUPILOT_GAZEBO_ROOT}/build && export GZ_SIM_RESOURCE_PATH=${WS_ROOT}/src/asv_wave_sim/gz-waves-models/models:${WS_ROOT}/src/asv_wave_sim/gz-waves-models/world_models:${SITL_MODELS_ROOT}/models:${SITL_MODELS_ROOT}/worlds:${ARDUPILOT_GAZEBO_ROOT}/models:${ARDUPILOT_GAZEBO_ROOT}/worlds && export RMW_IMPLEMENTATION=rmw_fastrtps_cpp && cd ${WS_ROOT}/src/asv_wave_sim/gz-waves-models/worlds && gz sim -v 4 -r aquatonomous_world.sdf"
